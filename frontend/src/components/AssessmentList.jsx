...
const AssessmentList = ({ onViewDetail, userEmail }) => {
  ...
  const exportToExcel = async () => {
    ...
    // Stamp the Export
    worksheet.addRow(["AC Mobility Field Assessment Report"]);
    worksheet.addRow([`Generated on: ${new Date().toLocaleString()}`]);
    worksheet.addRow([`Generated by: ${userEmail || 'Unknown User'}`]);
    worksheet.addRow([`Filters Applied: ${searchTerm ? `Search: "${searchTerm}"` : 'None'}`]);
    ...
    // Add Headers manually since we used addRow earlier
    ...
    // Style the Header Row (Now row 6 due to stamps)
    ...
    worksheet.eachRow((row, rowNumber) => {
      if (rowNumber <= 6) return; // Skip stamps and header
      row.eachCell((cell) => {
